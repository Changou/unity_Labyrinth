...
public class VirusQManager : MonoBehaviour
{
    ...
    void SetAnswers()
    {
        ...
        {
            options[i].GetComponent<VirusAnswerScript>().isCorrect = false;
            options[i].transform.GetChild(0).GetComponent<Text>().text = QnA[currentQuestion].Answers[i];

            if (QnA[currentQuestion].CorrectAnswer == i + 1)
            {
                options[i].GetComponent<VirusAnswerScript>().isCorrect = true;
            }
        }
    }
