...
public class NetworkQManager : MonoBehaviour
{
    ...
    void SetAnswers()
    {
        ...
        {
            options[i].GetComponent<NetworkAnswerScript>().isCorrect = false;
            options[i].transform.GetChild(0).GetComponent<Text>().text = QnA[currentQuestion].Answers[i];

            if (QnA[currentQuestion].CorrectAnswer == i + 1)
            {
                options[i].GetComponent<NetworkAnswerScript>().isCorrect = true;
            }
        }
    }
